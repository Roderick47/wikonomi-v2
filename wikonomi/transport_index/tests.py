...
class TransportIndexAccessTests(TestCase):
    def test_robots_txt_is_available(self):
        ...
        self.assertContains(response, 'Disallow: /transport/*/contact/')
        self.assertContains(response, 'Sitemap: http://www.wikonomi.com/sitemap.xml')

    def test_public_user_sees_coming_soon_page_while_disabled(self):
    ...
    def test_authenticated_user_sees_coming_soon_page_while_disabled(self):
    ...
    def test_staff_user_sees_admin_preview_while_disabled(self):
        user = get_user_model().objects.create_user(
...
@override_settings(
    WIKONOMI_BASE_URL='https://wikonomi.com',
    TRANSPORT_INDEX_PUBLIC_ENABLED=True,
)
class TransportRouterTests(TestCase):
    ...
    def test_registered_driver_location_update_is_deterministic(self, send_text):
        ...
        self.assertTrue(
            RouterEvent.objects.filter(
                stage=RouterEvent.Stage.LOCATION_UPDATE,
            ).exists()
        )
    ...
    def test_driver_signup_wizard_creates_minimal_driver(self, send_text):
        ...
        self.assertEqual(
            driver.profile_completeness,
            CabDriver.ProfileCompleteness.MINIMAL,
        )
        self.assertEqual(driver.status.availability, CabStatus.Availability.OFFLINE)
        self.assertIn('/transport/setup/', send_text.call_args_list[-1].args[1])
        self.assertEqual(LLMFallbackLog.objects.count(), 0)

    @patch('transport_index.router.send_text')
    @patch('transport_index.router.send_interactive_list')
    def test_rider_location_returns_verified_contact_proxy_links(
        self,
        send_interactive_list,
        send_text,
    ):
        verified = self._driver(
            phone='67570000003',
            verified=True,
            display_name='Verified Taxi',
        )
        unverified = self._driver(
            phone='67570000004',
            verified=False,
            display_name='Unverified Taxi',
        )
        ...
        self.assertIn(
            reverse('transport_index:cab_contact', args=[verified.slug]),
            link_message,
        )
        ...
        self.assertTrue(
            RouterEvent.objects.filter(
                stage=RouterEvent.Stage.RIDER_LOCATION,
            ).exists()
        )
        self.assertEqual(LLMFallbackLog.objects.count(), 0)
    ...
    def test_gazetteer_hit_avoids_llm(self, send_interactive_list, send_text):
        ...
        self.assertTrue(
            RouterEvent.objects.filter(
                stage=RouterEvent.Stage.GAZETTEER_HIT,
            ).exists()
        )
        self.assertEqual(LLMFallbackLog.objects.count(), 0)
    ...
    @patch('transport_index.router.send_text')
    @patch(
        'transport_index.llm_fallback.extract_intent',
        return_value={'intent': 'unknown', 'place': ''},
    )
    def test_llm_fallback_is_only_final_route(self, extract_intent, send_text):
        ...
        self.assertTrue(
            RouterEvent.objects.filter(
                stage=RouterEvent.Stage.LLM_FALLBACK,
            ).exists()
        )


@override_settings(TRANSPORT_INDEX_PUBLIC_ENABLED=True)
class TransportViewTests(TestCase):
    ...
    def test_unverified_profile_is_directly_reachable_without_phone_in_source(self):
        response = self.client.get(
            reverse('transport_index:cab_profile', args=[self.unverified.slug])
        )
    ...
    def test_contact_proxy_logs_attempt_and_redirects_to_whatsapp(self):
        ...
        self.assertTrue(
            response['Location'].startswith(
                f'https://wa.me/{self.verified.whatsapp_number}'
            )
        )
...
class StaleStatusTests(TestCase):
    def test_set_stale_statuses_offline_only_updates_old_available_statuses(self):
        ...
        stale_status = CabStatus.objects.create(
            driver=stale_driver,
            availability=CabStatus.Availability.AVAILABLE,
        )
        fresh_status = CabStatus.objects.create(
            driver=fresh_driver,
            availability=CabStatus.Availability.AVAILABLE,
        )
        CabStatus.objects.filter(id=stale_status.id).update(
            last_updated=timezone.now() - timezone.timedelta(minutes=25)
        )
...
class PriorityTwoObservabilityTests(TestCase):
    ...
    def test_llm_fallback_logs_estimated_cost(self, post):
        ...
        response.json.return_value = {
            'content': [{
                'type': 'text',
                'text': '{"intent": "find_cab", "place": "Waigani"}',
            }],
    ...
    def test_router_event_admin_dashboard_renders_observability_counts(self):
        ...
        RouterEvent.objects.create(
            phone_number='67570000042',
            stage=RouterEvent.Stage.KEYWORD_MATCH,
            message_type='text',
        )
        ...
        response = self.client.get(
            '/admin/transport_index/routerevent/dashboard/'
        )
...
class PriorityThreeUXTests(TestCase):
    ...
    def test_cab_list_second_page_preserves_results(self):
        ...
        response = self.client.get(
            reverse('transport_index:cab_list'),
            {'page': '2'},
        )
    ...
    def test_profile_completion_form_rejects_large_images(self):
        ...
        form = ProfileCompletionForm(
            ...
            data={'bio': 'Hello', 'home_area': 'Waigani'},
        )
