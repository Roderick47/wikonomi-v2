...
class LLMFallbackLogAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        extra_context['monthly_cost'] = (
            LLMFallbackLog.objects.filter(created_at__gte=month_start).aggregate(total=Sum('estimated_cost_usd'))['total']
            or 0
        )
...
class RouterEventAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('created_at',)
    change_list_template = 'admin/transport_index/routerevent/change_list.html'
    ...
    def dashboard(self, request):
        ...
        outbound = WhatsAppMessageLog.objects.filter(created_at__gte=since).values('status').annotate(count=Count('id')).order_by('status')
        llm_cost = LLMFallbackLog.objects.filter(created_at__gte=since).aggregate(total=Sum('estimated_cost_usd'))['total'] or 0
        return TemplateResponse(
            request,
            'admin/transport_index/router_dashboard.html',
            {'rows': rows, 'total': total, 'outbound': outbound, 'llm_cost': llm_cost},
        )


@admin.register(WhatsAppMessageLog)
class WhatsAppMessageLogAdmin(admin.ModelAdmin):
    list_display = ('created_at', 'recipient', 'message_type', 'status')
    list_filter = ('status', 'message_type', 'created_at')
    search_fields = ('recipient',)
    readonly_fields = ('created_at', 'payload', 'response', 'error')
