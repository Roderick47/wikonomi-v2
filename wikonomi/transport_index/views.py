...
from .whatsapp_client import get_webhook_verify_token, get_whatsapp_setting
...
if importlib.util.find_spec('django_ratelimit'):
    ...
    raise ImproperlyConfigured(
        'django-ratelimit must be installed for production contact protection.'
    )
else:
...
def transport_index(request):
    """Serve the directory when enabled, otherwise retain the paused preview."""
    if getattr(settings, 'TRANSPORT_INDEX_PUBLIC_ENABLED', False):
        return cab_list(request)
...
def _valid_meta_signature(request):
    app_secret = get_whatsapp_setting('WHATSAPP_APP_SECRET', '')
    if not app_secret:
        return True
...
def whatsapp_webhook(request):
    ...
    for inbound_message in _parse_inbound_messages(payload):
        dispatch_inbound_message(
            inbound_message,
            metadata=inbound_message.get('metadata'),
        )
        dispatched += 1
...
def cab_list(request):
    drivers = CabDriver.objects.select_related('status').filter(
        is_verified=True,
...
def cab_profile(request, slug):
    driver = get_object_or_404(
        CabDriver.objects.select_related('status'),
        slug=slug,
    )
    return render(
        request,
        'transport_index/cab_profile.html',
        {'driver': driver},
    )
...
def setup_profile(request, token):
    try:
        data = signing.loads(
            token,
            salt='transport-index-setup',
            max_age=60 * 60 * 24 * 7,
        )
    except signing.BadSignature:
        return render(
            request,
            'transport_index/setup_invalid.html',
            status=403,
        )
    ...
    return render(
        request,
        'transport_index/setup_profile.html',
