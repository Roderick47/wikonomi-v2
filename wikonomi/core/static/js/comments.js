(function () {
  ...
  const state = {
    ...
    flagTargetId: null, flagReason: 'other', isSubmittingFlag: false, toast: ''
  };
  ...
  function renderComment(c) {
    ...
    return `<article class="wk-comments__item" id="comment-${c.id}" data-id="${c.id}">
      ...
      ${isEditing ? `<div class="wk-comments__edit"><textarea data-input="edit" data-id="${c.id}">${esc(state.editBody)}</textarea><div><button data-action="save-edit" data-id="${c.id}">Save</button><button data-action="cancel-edit">Cancel</button></div></div>` : `<p>${esc(c.body)}</p>`}
      <div class="wk-comments__actions">
        ...
        <button data-action="more" data-id="${c.id}">More</button>
      </div>
      ${showMenu ? `<div class="wk-comments__menu">
  ...
  const renderDeleteModal = () => state.deleteTargetId ? `<div class="wk-comments__modal"><div class="wk-comments__modal-content"><p>Delete this comment?</p><button data-action="confirm-delete" data-id="${state.deleteTargetId}">Delete</button><button data-action="cancel-delete">Cancel</button></div></div>` : '';
  const renderFlagModal = () => state.flagTargetId ? `<div class="wk-comments__modal"><div class="wk-comments__modal-content"><p>Flag comment</p>
    <label><input type="radio" name="flag-reason" value="spam" ${state.flagReason==='spam'?'checked':''}/> Spam</label>
    <label><input type="radio" name="flag-reason" value="abuse" ${state.flagReason==='abuse'?'checked':''}/> Abuse</label>
  ...
  async function loadComments(append = false) { /* unchanged */
    ...
    } finally { state.isLoading = false; render(); maybeScrollToHash(); }
  }
  ...
  function maybeScrollToHash() {
    ...
    el.classList.add('wk-comments__highlight');
    setTimeout(() => el.classList.remove('wk-comments__highlight'), 1800);
  }
  ...
  root.addEventListener('click', async (e) => {
    ...
    if (action === 'compose') { const body = (root.querySelector('[data-input="compose"]')?.value || '').trim(); if (!body) return; const res = await api('', { method: 'POST', body: JSON.stringify({ content_type: Number(ct), object_id: Number(oid), body }) }); if (res.ok) loadComments(); }
    if (action === 'send-reply') { const body = (root.querySelector(`[data-input="reply"][data-id="${id}"]`)?.value || '').trim(); if (!body) return; const res = await api(`${id}/reply/`, { method: 'POST', body: JSON.stringify({ body }) }); if (res.ok) { state.openReplyFormId = null; loadComments(); } }
    ...
    if (action === 'edit') { const cur = state.comments.find((c) => c.id === id); state.editingCommentId = id; state.editBody = cur?.body || ''; return render(); }
    if (action === 'cancel-edit') { state.editingCommentId = null; state.editBody = ''; return render(); }
    if (action === 'save-edit') { const body = (root.querySelector(`[data-input="edit"][data-id="${id}"]`)?.value || '').trim(); if (!body) return; const res = await api(`${id}/`, { method: 'PATCH', body: JSON.stringify({ body }) }); if (res.ok) { state.comments = state.comments.map((c) => c.id === id ? { ...c, body, is_edited: true } : c); state.editingCommentId = null; render(); } }
